...
# --- CORREÇÃO DO NAMEERROR: Imports no topo ---
...
# --- CORREÇÃO DO 404: Esta é a URL "Raw" correta ---
...
def carregar_dados_biomas():
    ...
    for bioma in nomes_biomas:
        ...
        try:
            ...
            # Mostra um erro mais claro se a URL falhar
            st.warning(f"Não foi possível carregar {url_csv}. Erro: {e}")
            pass
    ...
    try:
        ...
        st.warning("Formato de data não é AAAA/MM. Tentando converter de outra forma...")
        try:
            df_completo['date'] = pd.to_datetime(df_completo['date'])
            df_completo['ano'] = df_completo['date'].dt.year
            df_completo['mes'] = df_completo['date'].dt.month
        except Exception as e:
            st.error(f"Erro fatal ao processar a coluna 'date': {e}")
            return pd.DataFrame()
...
@st.cache_resource  # Usamos cache_resource para o objeto do modelo
def treinar_modelo():
...
if modelo is None:
    st.error("Falha ao carregar os dados. A página de previsão não pode funcionar.")
    st.stop()
...
with col3:
    ano_maximo = carregar_dados_biomas()['ano'].max()
    ano_selecionado = st.number_input("Insira o Ano", min_value=2000, max_value=2050, value=ano_maximo + 1)
